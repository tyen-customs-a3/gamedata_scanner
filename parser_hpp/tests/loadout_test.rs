...
fn test_loadout_parsing() {
    ...
    for (item, parent) in uniform_items {
        ...
        assert!(class.properties.iter().any(|p| p.name == "author" && matches!(&p.value, HppValue::String(s) if s == "BW")));
        assert!(class.properties.iter().any(|p| p.name == "scope" && matches!(&p.value, HppValue::Number(n) if *n == 2)));
        ...
        if let HppValue::Class(info_class) = &item_info.value {
            assert_eq!(info_class.parent.as_deref(), Some("UniformItem"), "ItemInfo should inherit from UniformItem");
            assert!(info_class.properties.iter().any(|p| p.name == "uniformModel" && matches!(&p.value, HppValue::String(s) if s == "-")));
            assert!(info_class.properties.iter().any(|p| p.name == "uniformClass"));
            assert!(info_class.properties.iter().any(|p| p.name == "containerClass" && matches!(&p.value, HppValue::String(s) if s == "Supply60")));
            assert!(info_class.properties.iter().any(|p| p.name == "mass" && matches!(&p.value, HppValue::Number(n) if *n == 40)));
        } else {
